...
The single-issue-mode handoff is in [`execute-change-control/references/multi-env-execution.md`](../../execute-change-control/references/multi-env-execution.md). The plan-side just needs to enumerate the envs and the indicators correctly; the executor handles the orchestration.
