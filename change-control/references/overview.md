...
The plan-side skill enumerates both. The execute-side skill confirms or refutes both at each checkpoint.
...
- **Error budget** (SRE book ch. 3) — the inverse of an SLO. If your availability SLO is 99.9%, you have a 0.1% error budget per window. Risky changes burn more of it than safe changes; the risk-tier rubric in `plan-change-control/references/risk-rubric.md` maps that to checkpoint cadence.
...
- For the plan/execute decision tree, read [decision-tree.md](decision-tree.md).
- For known limits of the local skill, read [limitations.md](limitations.md).
- For the methodology applied to a specific change, invoke `plan-change-control` against a diff or PR url.
