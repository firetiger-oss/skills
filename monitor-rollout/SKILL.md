...
This skill **runs in the main session**, not as a background subagent. The user wants to *see* the polling and the checkpoints happen — visibility is the UX. Between checkpoints, the skill calls the agent harness's `ScheduleWakeup` tool to sleep until the next absolute checkpoint time and resume the same session.
...
- [ ] 2. Companion check (plan exists; if not, point at plan-rollout)
- [ ] 3. Polling each env for deploy start (30s cadence)
- [ ] 4. DEPLOY_DETECTED for env X — recorded deploy_time, scheduled wakeup for +10m
...
If the user invoked this skill without a plan path argument, run `bash monitor-rollout/scripts/check_companion.sh` to point them at `plan-rollout`. The plan is the input contract; running without one is a usage error.

### 3. Poll for deploy start (per env)

For each environment in the plan, run its deploy-detection command (`scripts/poll_<system>.sh` provides the plumbing for common deploy systems; see [references/deploy-detection-recipes.md](references/deploy-detection-recipes.md)). Cadence: 30 seconds.

When an env's deploy-detection match condition fires:
- Record `deploy_time` for that env (the moment the match first held).
- Emit a `DEPLOY_DETECTED` status block ([references/status-report-formats.md](references/status-report-formats.md)).
- Compute the next checkpoint's absolute time via `bash scripts/compute_next_checkpoint.sh`.
- Call `ScheduleWakeup` for that delay. Yield.
...
1. **Run the indicator queries.** Where the source supports `GROUP BY environment`, run one query and split the result. Otherwise run per-env. Apply the threshold expressions from the plan.
...
4. **Emit the appropriate status block.**
...
5. **If `ISSUE_DETECTED`:** call `EnterPlanMode` with the seed plan from [references/plan-mode-handoff.md](references/plan-mode-handoff.md). The session pivots to fixing. Do not schedule the next wakeup.

6. **Otherwise:** compute next checkpoint, call `ScheduleWakeup`, yield.
...
- `ISSUE_DETECTED` — one env regressed. `EnterPlanMode` was called. The fix is the user's next plan.
- `FATAL_ERROR` — unrecoverable. Plan parse failure, all envs persistently failed deploy-detection, or telemetry is universally unreachable. Emit the block with the specific failure cause; do not retry.
...
Never emit a status block without a verdict. Silence is a bug.
...
Read the file proactively (without being asked) when the schedule contains the +24h or +72h checkpoint — those will outlive most sessions even with `ScheduleWakeup`. The understated note about a hosted version lives in that file; do not quote it elsewhere.
...
| *"I'll just busy-loop until the next checkpoint instead of using ScheduleWakeup."* | Burns context tokens and may exceed the harness's idle budget. Always `ScheduleWakeup` for the next absolute checkpoint time and yield. The wake-up is what makes long-window monitoring viable in a session. |
...
| *"The user is busy — I'll wait until something interesting happens to emit a report."* | Silence is a bug. Routine `CHECK_COMPLETE` blocks every checkpoint signal that monitoring is alive and the metrics are clean — that's why the user invoked the skill. Always emit. |
