...
4. Stop monitoring across all envs. Other envs that also showed `regressed` are listed as "deferred — re-run /monitor-rollout after the current fix lands to resume monitoring on those envs."
...
T = +12m   ISSUE_DETECTED block emitted; EnterPlanMode called with seed:
             "Staging regressed: error rate 1.2% vs baseline 0.18%; cache hit ratio
              ...
              /monitor-rollout after the fix lands."
T = +12m   Session pivots to plan mode, building the fix.
```

After the fix lands and the user re-invokes `/monitor-rollout` against the same plan, the executor starts polling for the new deploy_time and runs the schedule from there.
