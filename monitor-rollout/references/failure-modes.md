...
**User remediation:** fix the plan file (or re-run `/plan-rollout`) and re-invoke.
...
**Action:** the executor doesn't pick up the edit (the plan is parsed once at start). The window continues with the originally-loaded plan. If the user wants the edit to take effect, they re-invoke `/monitor-rollout` against the updated plan; the new run starts polling again from scratch.
