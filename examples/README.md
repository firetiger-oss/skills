...
These are the kind of monitoring sections `plan-change-control` aims to produce. They serve two purposes:
...
Each example follows the template at `plan-change-control/assets/monitoring-plan-template.md`. They're purely illustrative — the indicator names, baselines, and thresholds are made-up plausible numbers; for a real change the planner would query the user's actual telemetry.
