...
- **Multiple PRs merged in quick succession.** The first PR's merge_sha may not equal the final deployed SHA. The executor handles ancestry (a merge sha is "deployed" if it's an ancestor of the deployed sha) — see [`plan-rollout` step 8 in the SKILL.md].
