...
**Recommended action:** roll back via `<rollback hint copied verbatim from the plan>`. Re-run `/execute-change-control <plan>` after the rollback to confirm metrics return to baseline.
...
The executor is stopping. Resolve the underlying issue and re-invoke `/execute-change-control <plan>`.
```
