...
**Recommended action:** roll back via \`$(field rollback_hint)\`. Re-run \`/execute-change-control $(field plan_path)\` after the rollback to confirm metrics return to baseline.
...
The executor is stopping. Resolve the underlying issue and re-invoke \`/execute-change-control $(field plan_path)\`.
EOF
        ;;
