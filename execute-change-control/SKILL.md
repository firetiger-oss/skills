...
- The user invoked this directly from `change-control` after the planning phase.
...
Input: path to a monitoring plan file produced by `plan-change-control`. The plan section's structure is documented in `plan-change-control/assets/monitoring-plan-template.md`.
...
After the fix is shipped, the user re-invokes `/execute-change-control` against the same plan; any still-present issue in another env triggers the next handoff.
...
```
execute-change-control progress:
- [ ] 1. Parsed plan (tier, indicators, schedule, deploy-detection per env, rollback)
- [ ] 2. Companion check (plan exists; if not, point at plan-change-control)
...
If the user invoked this skill without a plan path argument, run `bash execute-change-control/scripts/check_companion.sh` to point them at `plan-change-control`. The plan is the input contract; running without one is a usage error.
